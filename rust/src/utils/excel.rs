...
pub fn write_excel(filepath: &Path, orders: &[Order]) -> Result<(), String> {
    ...
    for (row_idx, order) in orders.iter().enumerate() {
        ...
        write_cell(&mut worksheet, row, 3, &order.user_name.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 4, &order.user_phone.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 5, &order.user_id_card.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 6, &order.user_email.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 7, &order.user_address.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 8, &order.product_id.to_string(), &data_format)?;
        write_cell(&mut worksheet, row, 9, &order.product_name.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 10, &order.product_category.as_ref().unwrap_or(&String::new()), &data_format)?;
        ...
        write_cell(&mut worksheet, row, 14, &order.discount_amount.as_ref().map(|d| d.to_string()).unwrap_or_else(|| "0".to_string()), &data_format)?;
        ...
        write_cell(&mut worksheet, row, 17, &order.payment_method.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 18, &format_optional_datetime(&order.payment_time), &data_format)?;
        write_cell(&mut worksheet, row, 19, &order.order_source.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 20, &order.shipping_address.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 21, &order.receiver_name.as_ref().unwrap_or(&String::new()), &data_format)?;
        write_cell(&mut worksheet, row, 22, &order.receiver_phone.as_ref().unwrap_or(&String::new()), &data_format)?;
