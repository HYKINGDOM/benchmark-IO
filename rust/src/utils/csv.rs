...
pub fn write_csv(filepath: &Path, orders: &[Order]) -> Result<(), String> {
    ...
    for order in orders {
        let row = format!(
            ...
            order.user_id,
            escape_csv_field(&order.user_name),
            escape_csv_field(&order.user_phone),
            escape_csv_field(&order.user_id_card),
            escape_csv_field(&order.user_email),
            escape_csv_field(&order.user_address),
            order.product_id,
            escape_csv_field(&order.product_name),
            escape_csv_field(&order.product_category),
            order.product_price,
            order.quantity,
            order.total_amount,
            ...
            escape_csv_field(&order.payment_method),
            format_optional_datetime(&order.payment_time),
            escape_csv_field(&order.order_source),
            escape_csv_field(&order.shipping_address),
            escape_csv_field(&order.receiver_name),
            escape_csv_field(&order.receiver_phone),
