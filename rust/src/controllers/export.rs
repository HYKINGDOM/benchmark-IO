        ...
        for offset in (0..total).step_by(batch_size as usize) {
            ...
            for order in orders {
                let row = format!(
                    ...
                    order.order_no,
                    order.user_id,
                    order.user_name.unwrap_or_default(),
                    order.user_phone.unwrap_or_default(),
                    order.user_id_card.unwrap_or_default(),
                    order.user_email.unwrap_or_default(),
                    order.user_address.unwrap_or_default(),
                    order.product_id,
                    order.product_name.unwrap_or_default(),
                    order.product_category.unwrap_or_default(),
                    order.product_price,
                    order.quantity,
                    order.total_amount,
                    order.discount_amount.map(|d| d.to_string()).unwrap_or_else(|| "0".to_string()),
                    order.pay_amount,
                    order.order_status,
                    order.payment_method.unwrap_or_default(),
                    order.payment_time.map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or_default(),
                    order.order_source.unwrap_or_default(),
                    order.shipping_address.unwrap_or_default(),
                    order.receiver_name.unwrap_or_default(),
                    order.receiver_phone.unwrap_or_default(),
