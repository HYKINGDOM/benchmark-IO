...
function init(args)
    print("Async Export Test initialized")
    print("  Format: " .. format)
...
function request()
    ...
    return wrk.format("POST", "/api/v1/exports/async", {
        ["X-API-Key"] = api_key,
...
function response(status, headers, body)
    if status == 200 or status == 201 then
        ...
        -- 可以解析 body 获取 task_id
        -- local task_id = parse_task_id(body)
        -- table.insert(task_ids, task_id)
    else
        print("Async Export Error: " .. status .. " - " .. body)
    end
end

function done(summary, latency, requests)
    print("\n=== Async Export Test Summary ===")
